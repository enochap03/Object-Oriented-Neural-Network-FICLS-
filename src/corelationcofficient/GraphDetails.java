...
public class GraphDetails {
	...
	public int getDeltas(String regExpression,String delnames[], double deltas [][]) {
		...
		for(int i=0;i<=getDeltaLimit();i++) {
			...
			if(Pattern.matches(regExpression, getDelName()[i])) {
				posMark[delPos] = i;
				delPos++;
				//return (convertListToArrayDouble(getDeltas()[i]));
			}
		}
		//delnames = new String[delPos];
		//deltas = new double [delPos][];
		for(int j=0;j<delPos;j++) {
			int pos = posMark[j];
			delnames[j] = getDelName()[pos];
			//deltas[j] = new double[getConnections()[pos].size()];
	...
	public int getConnectionsOfDelta(String expression, String conNames[],double connections [][]) {
		...
		//conNames = new String[num];
		//connections = new double[num][];
		for(int j=0;j<num;j++) {
			int pos = posMark[j];
			conNames[j] = getConName()[pos];
			//connections[j] = new double[getConnections()[pos].size()];
